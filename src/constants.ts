...
/** Root managed dir (lock + installed hooks). Override with AGENTHOOKS_HOME (used by tests). */
export const MANAGED_DIR =
  process.env.AGENTHOOKS_HOME ?? join(homedir(), ".agents");

/** Global lock — the book of record for installed packages. */
export const GLOBAL_LOCK_PATH = join(MANAGED_DIR, "agenthooks.json");
